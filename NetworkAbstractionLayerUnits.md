...
H.264 스트림을 저장하는데 자주 사용되는 다른 한 가지 방법은 AVCC 형식이다. 이 형식에서 각 NALU는 자신의 크기정보 뒤에 따라오게 된다. (빅 엔디안 형식이다) 이 방법은 파싱하기가 쉽지만 Annex B에서의 바이트 정렬을 잃게 된다.  그저 일을 복잡하게 하기위해서 NAL의 길이는 1,2 또는 4바이트로 인코딩된다. 이 값은 헤더 부분에 저장되며 헤더는 보통 'extra data' 또는 시퀀스헤더로 불리운다. 아래는 해당 포맷의 기본형이다.
...
눈치챗겠지만 SPS 와 PPS는 이제 밴드 바깥에 저장되게 된다. 그것은 elementary stream 데이터와 분리된것을 의미한다. 저장소와 데이터의 전달은 파일 컨테이너의 몫이 되고 문서의 바깥부분에 위치하게 되는 것이다. 비록 우리가 스타트 코드는 쓰지않아도 emulation prevention은 여전히 있다는 것을 알아두어야한다.

추가적으로 새로운 변수 NALULengthSizeMinusOne이 있다. 이것은 헷갈리게도 얼마나 많은 바이트를 각 NALU의 길이를 저장하는데에 쓸 수 있는지 나타내는 것이다. 그러니까 만약 NALULengthSizeMinusOne가 0이면 각 NALU는 해당 NALU의 길이를 나타내는 1개 바이트 뒤에 따르게 되는 것이다. 사이즈를 저장하는 이 한 바이트를 이용하며, 최대 크기는 255 바이트이다. 모든 키프레임을 저장하기에는 사실상 작은 크기에 불과하다. 2 바이트는 64k 의 NALU 를 제공해 줄 수 있고 이것은 예에서 나타난다. 하지만 이것도 작다. 3바이트면 완벽할 것이다. 하지만 어떤 이유에선지 보편적으로 사용되지 않는다. 그러므로 4바이트가 지금까지는 가장 보편화되어있고 우리가 여기서 사용할 것이다.
...
이 형식의 장점이라면 디코더가 시작점에서 스트림의 중간지점가지 점프할 수 있다는 것이다. 이것은 미디어가 하드드라이브 같은 랜덤 액세스 매체에서 사용되어지며 그러므로 MP4, MKV 같은 컨테이너 형식에서 사용되어지는 것이다.
